...
from ..core.visualizer import (show_result, show_bev_result_kitti)
...
class CustomKittiDataset(KittiDataset):
    ...
    def show(self, results, out_dir, show=True, pipeline=None):
        ...
        show_threshold = 0.45
        save_imgs = True
        # f = open("/mnt/hdd4/achieve-itn/PhD/Code/workdirs/Results"
        #          "/RetFormer/kitti/showval.txt", "a")
        ...
        for i, result in tqdm(enumerate(results)):
            ...
            # name filtering Pred
            labels = list(range(2))
            pred_bboxes_mask = np.array([n in labels for n in pred_labels],
                                      dtype=np.bool_)
            pred_bboxes_numpy = pred_bboxes_numpy[pred_bboxes_mask]
            pred_labels = pred_labels[pred_bboxes_mask]
