...
from ..utils.gated_linear_unit import GLU
from ..utils.multiscale_retention import MultiScaleRetention, MultiScaleRetentionSoftmax


class RetFormerEncoderLayer(nn.Module):
    def __init__(
            self,
            embed_dim,
            num_heads,
    ...
    def forward(self, x, pe, coords):
        ...
        # (h, )
        dist_2d_xn = coords[:, :, 3].unsqueeze(2)  # (n_g, g, 1)
        dist_2d_xm = coords[:, :, 3].unsqueeze(1)  # (n_g, 1, g)
...
class RetFormerEncoderBlock(nn.Module):
    def __init__(
            self,
            embed_dim,
            num_heads,
...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, coors, dtype):
        ...
        pex = x[:, None] / inv_freq()[None, :]  # (BM, d/2)
        pey = y[:, None] / inv_freq()[None, :]  # (BM, d/2)

        # [num_tokens, pos_length]
        pex = torch.stack([pex[:, ::2].sin(), pex[:, 1::2].cos()],
                          dim=-1).flatten(1)  # (BM, d/2)
        pey = torch.stack([pey[:, ::2].sin(), pey[:, 1::2].cos()],
                          dim=-1).flatten(1)  # (BM, d/2)
        pe = torch.cat([pex, pey], dim=-1).to(dtype)  # (BM, d)

        gap = self.feat_dim - pe.size(1)
        if gap > 0:
            pe_p = torch.zeros((pe.size(0), gap), dtype=dtype,
                               device=coors.device)
    ...
    def inv_freq(self):
        ndim = 2
        pos_length = (self.feat_dim // (ndim * 2)) * 2  # feat_dim/2
...
class RetFormer(nn.Module):
    """
    Embracing Point Cloud Transformer with Retentive Network
    """
    ...
    def __init__(
        ...
        self.embedding = PositionalEmbedding(embed_dim, sparse_shape,
        ...
        for _ in range(num_blocks):
            self.block_list.append(RetFormerEncoderBlock(embed_dim,
                                                         num_heads,
                                                         activation,
                                                         group_size))
    ...
    def recover_bev(self, voxel_feat, coors, batch_size):
        ...
        for batch_itt in range(batch_size):
            ...
            canvas = torch.zeros(feat_dim, nx * ny, dtype=voxel_feat.dtype,
